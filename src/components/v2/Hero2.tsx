  ...
  return (
    <section id="top" className="relative overflow-hidden border-b border-[var(--v2-border)]">
      <div className="absolute inset-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[var(--v2-bg)] via-[var(--v2-bg)]/85 to-[var(--v2-bg)]/20" />
      ...
      <div ref={ref} className="v2-reveal relative mx-auto max-w-6xl px-5 pb-24 pt-16 sm:px-8 sm:pt-24">
        <p className="v2-mono flex items-center gap-2 text-[12px] uppercase tracking-[0.18em] text-[var(--v2-green)]">
          <span className="h-2 w-2 rounded-full bg-[var(--v2-green)] shadow-[0_0_8px_var(--v2-green)]" />
          Diagnostics online
        </p>

        <h1 className="mt-5 max-w-2xl text-[clamp(2.4rem,6vw,4.4rem)] font-bold leading-[1.03] text-[var(--v2-ink)]">
        ...
        <div className="v2-mono mt-14 grid max-w-2xl grid-cols-2 gap-x-8 gap-y-6 sm:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="border-l-2 border-[var(--v2-amber)] pl-3">
              <dt className="text-[10px] uppercase tracking-[0.1em] text-[var(--v2-ink-faint)]">
                {stat.label}
              </dt>
