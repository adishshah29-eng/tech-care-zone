...
const fieldClass =
  "w-full border-0 border-b-2 border-dashed border-[var(--paper-line-strong)] bg-transparent py-2.5 text-[0.95rem] text-[var(--ink)] placeholder:text-[var(--ink-faint)] focus:border-[var(--brand)] focus:outline-none";
        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-6">
            ...
            <ContactRow icon={Phone} label="Call Us">
              <a href={`tel:${business.phoneHref}`} className="hover:text-[var(--brand)]">
                {business.phone}
              </a>
              <span className="block text-[var(--ink-faint)]">Mon–Sat · 10am–8pm</span>
            </ContactRow>
            <ContactRow icon={MessageCircle} label="WhatsApp">
              <a href={business.whatsappHref} target="_blank" rel="noreferrer" className="hover:text-[var(--brand)]">
                {business.phone}
              </a>
            ...
            {submitted ? (
              <div className="mt-8 border border-dashed border-[var(--green)] p-6 text-center">
                ...
                <p className="mt-2 text-[0.95rem] text-[var(--ink-soft)]">
                  WhatsApp should open with your details filled in, just hit send. <a href={waUrl} target="_blank" rel="noreferrer" className="underline">Didn&apos;t open? Tap here.</a> For anything urgent, call or
              ...
              <form onSubmit={handleSubmit} className="mt-6 grid gap-6 sm:grid-cols-2">
                ...
                <button
                  type="submit"
                  className="stamp-btn inline-flex w-fit items-center gap-2 rounded-sm bg-[var(--brand)] px-6 py-3 font-spec text-[14px] font-semibold uppercase tracking-[0.06em] text-[var(--paper-raised)] transition-transform hover:-rotate-1 sm:col-span-2"
                >
                  Send Repair Request
                  <ArrowRight size={16} />
                </button>
...
function ContactRow({
  ...
  return (
    <div className="flex gap-4">
      <Icon size={20} strokeWidth={1.75} className="mt-1 shrink-0 text-[var(--brand)]" />
      <div>
